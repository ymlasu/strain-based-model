function[sig,tau,sx,tx]=nonproportional_step(K, n, r, sig_y, sa, satime, ta, tatime, E, Epoisson)
sig=[0];
tau=[0];
...
timecountta=1;
for k=1:360
    if k>tatime(timecountta+1)
        timecountta=timecountta+1;
        tx(k)=ta(timecountta)+(ta(timecountta+1)-ta(timecountta))/(tatime(timecountta+1)-tatime(timecountta))*(k-tatime(timecountta));
    else
        tx(k)=ta(timecountta)+(ta(timecountta+1)-ta(timecountta))/(tatime(timecountta+1)-tatime(timecountta))*(k-tatime(timecountta));
    end
    if k>satime(timecountsa+1)
        timecountsa=timecountsa+1;
        sx(k)=sa(timecountsa)+(sa(timecountsa+1)-sa(timecountsa))/(satime(timecountsa+1)-satime(timecountsa))*(k-satime(timecountsa));
    else
        sx(k)=sa(timecountsa)+(sa(timecountsa+1)-sa(timecountsa))/(satime(timecountsa+1)-satime(timecountsa))*(k-satime(timecountsa));
    end
end
for j=1:3
    sx(end+1:end+360)=sx(1:360);
    tx(end+1:end+360)=tx(1:360);
end
